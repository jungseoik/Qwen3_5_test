...
├── .env.describe              # description 실행 전용 서버 오버레이 (.env 위에 덮음)
...
│   ├── dataset/               # 데이터셋 통계
│   │   └── stats.py
│   ├── evaluation/            # 오탐감소율 평가 + description 진단
│   │   ├── prompts.py         # VQA(yes/no) + description(영어) 프롬프트 (편집 대상)
...
│   └── analysis/            # 오분류 description 분석 도구 (웹)
│       ├── config.py         # 경로/포트 설정 (편집 대상)
│       ├── describe_server.py # http.server 백엔드 + CLI
│       └── static/index.html # 모델별·FN/FP별 갤러리 UI
└── docs/                      # 각 도구 상세 문서
    ├── dataset_stats.md
...
라벨 평가는 모델이 `yes`/`no` 한 토큰만 답해 **왜 틀렸는지** 알 수 없습니다. 특히 진짜 사건을 `no`로 걸러버리는 **FN(놓친 정탐)**이 많으면 정탐유지율이 떨어집니다. 이 도구는 오분류 케이스에 대해 vLLM 모델이 장면을 **영어로 묘사**하게 한 뒤 **Google Gemini API로 한국어 번역**하고(작은 모델의 한글 직생성 깨짐 회피), 그 설명을 이미지·프롬프트·정답과 나란히 넘겨보며 원인(인식 실패 vs 프롬프트 과민/둔감)을 진단합니다. 평가가 만든 `manifest.csv`를 입력으로 쓰므로 **평가 → description 생성 → Web UI** 3단계 순서입니다. 번역 키는 `.env`의 `GEMINI_API_KEY`(gitignore)에 넣고 `python src/evaluation/translate.py --selftest`로 확인합니다.

```bash
# 1) description 생성 (평가 결과 폴더의 fp/fn 케이스 대상)
python src/evaluation/describe_eval.py  --from results/sweep_labeled_<TS>/Qwen3.5-4B   # 단일 모델
python src/evaluation/describe_sweep.py --from results/sweep_labeled_<TS>              # 다중 모델

# 2) 분석 Web UI (기동 후 브라우저에서 http://localhost:8801)
python src/analysis/describe_server.py  --path results/sweep_labeled_<TS>
```
